    ...
    impl EnrollmentQuery {
        ...
        pub fn add_users(mut self, mut users: Vec<User>) -> EnrollmentQuery {
            ...
            let _ = self.users.iter_mut().map(|usersfilter| usersfilter.append(&mut users));
            self
        }
    }
    ...
    pub trait EnrollmentService {
        ...
        fn get_enrollment(&self, user: &User, course: &CourseKey) -> Result<Option<Enrollment>> {
            let mut enrollments = self.query_enrollment(&EnrollmentQuery::default().add_courses(&vec![course.clone()]).add_users(vec![user.clone()]))?;
